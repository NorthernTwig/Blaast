...
router
  .get('users', async (ctx) => {
    ...
    const users = await userDAL.getAll(limit, offset)
  ...
  .get('users/:_id', async (ctx) => {
    ...
    const userInfo = await userDAL.getOne(_id)
      .catch(() => ctx.throw('Could not find a user with that id', 404))
    ctx.body = generateSelf(userInfo, ctx)
  })
  .post('users', createUserCheck, async (ctx) => {
    const { username, name } = ctx.request.body
    const newUser = await userDAL.create(ctx.request.body)
      .catch((e) => {
        e.code === 11000 ?
    ...
    ctx.status = 201
    ctx.set('Location', `${ process.env.URL }/users/${ id }`)
    ctx.body = {
      status: ctx.status,
  ...
  .patch('users', updateUserCheck, jwt, async (ctx) => {
    ...
    await userDAL.update(_id, ctx.request.body)
    .catch((e) => {
      e.code === 11000 ?
        ctx.throw('Username is already taken', 409) :
