...
router
  .get('posts', async (ctx) => {
    ...
    const posts = await postDAL.getAll(limit, offset)
  ...
  .get('posts/:_id', async (ctx) => {
    ...
    const postInfo = await postDAL.getOne(_id)
  ...
  .get('posts/users/:_id', async (ctx) => {
    ...
    const limit = parseInt(ctx.query.limit, 1) || 10
    const offset = parseInt(ctx.query.offset, 1) || 0
    const posts = await postDAL.getUsersPost(_id, limit, offset)
      .catch((e) => {
  ...
  .post('posts', createPostCheck, jwt, async (ctx) => {
    const newPost = await postDAL.create(ctx)
      .catch(() => ctx.throw('Could not create post', 400))
    const { _id: id } = newPost
    ctx.set('Location', `${ process.env.URL }/posts/${ id }`)
    ctx.status = 201
    ctx.body = {
      status: ctx.status,
  ...
  .patch('posts/:_id', updatePostCheck, jwt, async (ctx) => {
    const updatedPost = await postDAL.update(ctx)
  ...
  .delete('posts/:_id', deletePostCheck, jwt, async (ctx) => {
    const deletedPost = await postDAL.remove(ctx)
