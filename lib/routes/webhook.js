...
import * as webhookDAL from '../DAL/webhook'
...
router
  .get('webhooks', jwt, async (ctx) => {
    ...
    const webhooks = await webhookDAL.getAll(limit, offset, _id)
  ...
  .get('webhooks/:_id', jwt, async (ctx) => {
    const webhookInfo = await webhookDAL.getOne(ctx)
  ...
  .post('webhooks', webhookCheck, jwt, async (ctx) => {
    const newWebhook = await webhookDAL.create(ctx)
  ...
  .patch('webhooks/:_id', updateWebhookCheck, jwt, async (ctx) => {
    ...
    const updatedWebhook = await webhookDAL.update(ctx, body)
  ...
  .delete('webhooks/:_id', jwt, async (ctx) => {
    const deletedWebhook = await webhookDAL.remove(ctx)
